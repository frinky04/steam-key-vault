      ...
      <header className="mobile-header sticky top-0 z-20 border-b border-border bg-surface/95 backdrop-blur">
        <div className="mx-auto max-w-4xl px-4">
          <div className="mobile-header-bar flex h-12 items-center gap-6">
            ...
            <NavLinks role={user.role} className="hidden sm:flex" />
          ...
          <NavLinks role={user.role} className="mobile-nav sm:hidden" />
        </div>
      </header>
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-4 sm:py-6">{children}</main>
    </div>
  );
}
